...
as 2ln(n)/ln(ln(n)) while Pidx grows like a positive power of n divided by ln(n).
...
the interval to have had omega equal to Pidx, thus impossible for any product
of n(n+1), there, to have been prime-complete. Once minimum Pidx grows to exceed maximum omega 
...
continues on an increasing trend because the rate of increase of product Pidx is faster 
than product omega.
...
factorizations. This does not disturbe min_Delta or min_Pidx because high values of Pidx 
...
That just puts the crossover point to Pidx dominance a couple of intervals up the number line.
