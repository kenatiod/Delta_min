...
/*
Version 1.0  (derived from Delta_N2_min.c Version 3.0)
...
On Linux, compile with:
    gcc -O3 -std=c11 -fopenmp -o Delta_N3_min Delta_N3_min.c -lm

On Mac, try:
    gcc-15 -O3 -std=c11 -fopenmp -o Delta_N3_min Delta_N3_min.c -lm

or if needed:
    brew install libomp
    clang -O3 -std=c11 -Xpreprocessor -fopenmp -I/opt/homebrew/opt/libomp/include -o Delta_N3_min Delta_N3_min.c -L/opt/homebrew/opt/libomp/lib -lomp -lm

You can run it as:
    ./Delta_N3_min                         (starts at 1, runs 40 intervals, uses the first 100 prime divisors)
    ./Delta_N3_min  1000 1000 20 30        (starts at 1000, runs 20 doublings of size starting at 1000, uses 30 primes)
    ./Delta_N3_min  10000000000000 1000000000000  1  300   (starts at 10T, runs 1 interval of size 1T, uses 300 primes)

By Ken Clements, based on Delta_N2_min.c (April 10, 2026). N_3 adaptation May 2026.
*/
...
#define CHECKPOINT_FILE "Delta_N3_min_checkpoint.dat"
...
int main(int argc, char *argv[]) {
    ...
    char buf1[64];
    printf("\nDelta_N3_min: Calculating the smallest difference between the prime number index of the greatest prime divisor\n");
    if (intervals == 1) {
    ...
    FILE *csv = NULL;
    if (resuming) {
        csv = fopen("Delta_N3_min_data.csv", "a");
    } else {
        csv = fopen("Delta_N3_min_data.csv", "w");
        if (csv) {
