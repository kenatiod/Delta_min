...
between pi(GPF) of the product N_2 = n(n+1), and its Omega (number of prime divisors). 
The prime index of the greatest prime factor, pi(GPF), is called the "Iota"
in this code. Because n and n+1 are coprime, the Omega of their product is the sum of
their individual Omega values, and the Iota of the product is the larger Iota value of the
...
Having 64 bit integers is enough for finding crossover ranges.
...
unnecessarily factored twice (i.e. the Omega and Iota values are recycled). 

For each doubling interval (usually powers of 2) the processing is also looking for the 
maximum Omega and minimum Pdix for that interval. The individual values of the Omega 
and Iota jump rapidly each n, but the maximum product Omega grows slowly from interval
to interval, and the minimum Iota also grows interval to interval as a trend, but sags
back before surging forward. The maximum Omega side starts out greater than the minimum
Iota, but over many intervals, the minimum Iota catches up, and then exceeds maximum Omega.
...
in that sequence, ending in 633,555. For 633,555x633,556 Omega and Iota equal 8.
Numbers are prime-complete if, and only if, their Omega exactly equals their Iota. 
For the products of pairs of consecutive integers, Omega grows aporximately 
as 2ln(n)/ln(ln(n)) while Iota grows like a positive power of n divided by ln(n).

The significance of looking at the maximum Omega and minimum Iota is that if the minimum Iota 
is greater than the maximum Omega over an interval, then it is impossible for any point in
the interval to have had Omega equal to Iota, thus impossible for any product
of n(n+1), there, to have been prime-complete. Once minimum Iota grows to exceed maximum Omega 
in all successive intervals, the closed set of prime-complete products has been enumerated, 
and the A141399 sequence is terminated.

This code evolved from a very siimple version written in Python in which the sympy module
supplied the primefactors(n) function from which Omega equals len(primefactors(n)) and Iota
...
generate fast values for Omega and Iota when those values might displace either maximum Omega
or minimum Iota.

The minimum delta, Iota-Omega, counts the number of missing prime factors from a prime-complete
number. After n = 633,555 we see that minimum advance from zero, and hits 1 for the last time
at n = 80,061,344 and 2 for the last time at n = 1,109,496,723,125. The minimum delta value 
continues on an increasing trend because the rate of increase of product Iota is faster 
than product Omega.

To get a view beyond the very jumpy nature of Iota and Omega, we can look at the maximum value
of Omega and minimum value of Iota over the interval. For early intervals max_Omega can be 
high enough to be equal to minIota, so it is possible for the delta to be zero somewhere. However
if an interval has a min_Iota greater than the max_Omega, then it is not possible that a
prime-complete product exists anywhere in that interval. When the faster growth trend of
min_Iota outruns max_Omega for all greater intervals (at about n = 2^45), no more prime-complete
n(n+1) are ever possible.

The nature of the algorithm in this code gives accurate results for min_Delta and min_Iota, but
max_Omega has some uncertanty. This is because a portion of interval values will not have complete 
factorizations. This does not disturbe min_Delta or min_Iota because high values of Iota 
are involved. Usually the max_Omega is at a product where n and n+1 have about the same 
number of prime divisors, but one or both of those can have residues after partial factorization 
that still have a small number of primes, so for big numbers, max_Omega can be a few counts low. 
That just puts the crossover point to Iota dominance a couple of intervals up the number line.
...
In this case the Iota(n(n+1)) is well above 15 (it's 28693), so we do not need to know 
...
On Linux, compile with:
    gcc -O3 -std=c11 -fopenmp -o Delta_min_N2 Delta_min_N2.c -lm

On Mac, I use:
    gcc-15 -O3 -std=c11 -fopenmp -o Delta_min_N2 Delta_min_N2.c -lm
...
You can run it as:
    ./Delta_min_N2                         (starts at 1, runs 40 intervals, uses the first 100 prime divisors )
    ./Delta_min_N2  1000 1000 20 30        (starts at 1000, runs 20 doublings of size starting at 1000, uses 30 primes )
    ./Delta_min_N2  10000000000000 1000000000000  1  300   (starts at 10T, runs 1 interval of size 1T, uses the first 300 prime divisors )

By Ken Clements, April 10, 2026. Last rev May 6, 2026
*/
...
#define CHECKPOINT_FILE "Delta_min_N2_checkpoint.dat"
...
int main(int argc, char *argv[]) {
    ...
    char buf1[64];
    printf("\nDelta_min_N2: Calculating the smallest difference between the prime number index of the greatest prime divisor\n");
    if (intervals == 1) {
        printf("     of N_2 = n(n+1) and the count of prime divisors of that consecutive integer product, over an interval.\n");
    } else {
        printf("     of N_2 = n(n+1) and the count of prime divisors of that consecutive integer product, over doubling intervals.\n");
    }
    printf("===============================================================================================================\n\n");
    ...
    FILE *csv = NULL;
    if (resuming) {
        csv = fopen("Delta_min_N2_data.csv", "a");
    } else {
        csv = fopen("Delta_min_N2_data.csv", "w");
        if (csv) {
...
#pragma omp for schedule(static)
            ...
            for (uint64_t blk = 0; blk < num_blocks; blk++) {
                ...
                for (size_t i = 0; i < pair_count; i++) {
                    ...
                    io_result_t *a = &arr[i];
                    io_result_t *b = &arr[i + 1];

                    int Omega_lb = a->Omega_small + b->Omega_small;
                    if (a->exact_Iota && b->exact_Iota) {
                        int Iota = (a->Iota_small > b->Iota_small) ? a->Iota_small : b->Iota_small;
