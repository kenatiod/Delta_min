...
between pi(GPF) of the product N_3 = n(n+1)(n+2), and its Omega (number of distinct
prime divisors). The prime index of the greatest prime factor, pi(GPF), is called the
"Iota" in this code.

Because n, n+1, n+2 are not all mutually coprime in general, the Omega of their product
requires a small correction. When n is odd, the three integers are pairwise coprime, so
Omega(N_3) = Omega(n) + Omega(n+1) + Omega(n+2). When n is even, both n and n+2 are
even, so they share the prime factor 2; in that case one must be subtracted from the
sum: Omega(N_3) = Omega(n) + Omega(n+1) + Omega(n+2) - 1. No other prime can be shared
among the three, because any prime p dividing both n and n+2 would also divide their
difference 2, forcing p = 2.

The Iota of N_3 = n(n+1)(n+2) is the maximum Iota of the three integers. Computing
those values for n, n+1, and n+2 avoids forming the product (which could exceed 192
bits) and computing over larger numbers. Having 64-bit integers is enough for finding
crossover ranges. Progressing through successive triples, the values for n+1 and n+2
can be saved for reuse in the next triple (n+1)(n+2)(n+3), so successive numbers are
not unnecessarily factored twice.

For each doubling interval the processing is also looking for the maximum Omega and
minimum Iota for that interval. The minimum delta (Iota - Omega) is tracked over the
interval. Once the minimum Iota grows to exceed the maximum Omega in all successive
intervals, no product n(n+1)(n+2) can be prime-complete in those intervals.

The minimum Iota values are related to sequence A003032 at OEIS.org/A003032 listing:
...
On Linux, compile with:
    gcc -O3 -std=c11 -fopenmp -o Delta_min_N3 Delta_min_N3.c -lm

On Mac, try:
    gcc-15 -O3 -std=c11 -fopenmp -o Delta_min_N3 Delta_min_N3.c -lm

or if needed:
    brew install libomp
    clang -O3 -std=c11 -Xpreprocessor -fopenmp -I/opt/homebrew/opt/libomp/include -o Delta_min_N3 Delta_min_N3.c -L/opt/homebrew/opt/libomp/lib -lomp -lm

You can run it as:
    ./Delta_min_N3                         (starts at 1, runs 40 intervals, uses the first 100 prime divisors)
    ./Delta_min_N3  1000 1000 20 30        (starts at 1000, runs 20 doublings of size starting at 1000, uses 30 primes)
    ./Delta_min_N3  10000000000000 1000000000000  1  300   (starts at 10T, runs 1 interval of size 1T, uses 300 primes)

By Ken Clements, based on Delta_min_N2.c (April 10, 2026). N_3 adaptation May 2026.
*/
...
#define CHECKPOINT_FILE "Delta_min_N3_checkpoint.dat"
...
static int primes_length = 303; // Only a part of list is used for
        // n(n+1)(n+2). Larger list items used for longer consecutive integer products.
...
int main(int argc, char *argv[]) {
    ...
    char buf1[64];
    printf("\nDelta_min_N3: Calculating the smallest difference between the prime number index of the greatest prime divisor\n");
    if (intervals == 1) {
        printf("     of N_3 = n(n+1)(n+2) and the count of prime divisors of that consecutive integer product, over an interval.\n");
    } else {
        printf("     of N_3 = n(n+1)(n+2) and the count of prime divisors of that consecutive integer product, over doubling intervals.\n");
    }
    printf("=================================================================================================================\n\n");
    ...
    printf("Omega(n(n+1)(n+2)) = Omega(n)+Omega(n+1)+Omega(n+2) - (1 if n is even, else 0).\n");
    ...
    if (resuming) {
        ...
        // N_3(1) = 1*2*3 = 6 = 2*3.  Omega=2, Iota=pi(3)=2, delta=0.
        // n=1 is odd, so no subtraction for the shared factor of 2.
    ...
    FILE *csv = NULL;
    if (resuming) {
        csv = fopen("Delta_min_N3_data.csv", "a");
    } else {
        csv = fopen("Delta_min_N3_data.csv", "w");
        if (csv) {
            ...
            if (orig_interval_start == 1 && orig_initial_size == 1) {
                // N_3(1)=1*2*3=6: Omega=2, Iota=2, delta=0
...
#pragma omp parallel
#endif
        {
            ...
            // We need n, n+1, n+2 for each triple, so we allocate two extra slots.
            uint64_t total_numbers = (n_stop + 2) - n_start + 1;
...
#pragma omp for schedule(static)
            ...
            for (uint64_t blk = 0; blk < num_blocks; blk++) {
                ...
                // Need two extra slots beyond block_n to cover the n+2 element of the last triple.
                uint64_t block_end_excl = block_start + block_n + 2;
                if (block_end_excl > n_stop + 3) block_end_excl = n_stop + 3;
                ...
                // Each triple is (arr[i], arr[i+1], arr[i+2]) corresponding to n, n+1, n+2.
                size_t triple_count = (len >= 2) ? len - 2 : 0;
                for (size_t i = 0; i < triple_count; i++) {
                    ...
                    // Omega lower bound: sum of the three individual Omega values,
                    // minus 1 when n is even (n and n+2 both even, sharing the prime 2).
                    int shared = (n % 2 == 0) ? 1 : 0;
                    int Omega_lb = a->Omega_small + b->Omega_small + c->Omega_small - shared;

                    // Iota: the maximum prime index among the three integers.
                    if (a->exact_Iota && b->exact_Iota && c->exact_Iota) {
                        ...
                        if (a->exact_Omega && b->exact_Omega && c->exact_Omega) {
