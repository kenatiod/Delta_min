...
Number theory research program by Ken Clements (Started: February 10, 2026, last revised April 27, 2026).
...
Although Delta_min.c was originally written to iterate through multiple intervals that double in size, single intervals may be run starting at any n value with any length by setting the doubling count to one. For example, to catch that minimum=3 value mentioned above, run:
                         ./Delta_min 1109496723126 200000000000 1

See the file "Delta_min_Heuristic_Case.md" for a more detailed explanation, and the file "Delta_min_output.txt" for an output example to n = 200 trillion. The Python program "verify_delta_min.py" has been added to the repo so the Delta_min_output.txt file can be automatically checked for correct factorizations producting values for omega and Pidx.
